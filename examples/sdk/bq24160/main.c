...
uint8_t TestRegister(uint8_t test_val, uint8_t reg_addr) {

	BQ24160_IIC0_EWriteByte(BQ24160_ADDRESS, REG_STATUS, test_val);
	unsigned char rx_buff[2];
	BQ24160_IIC0_EReadByte(BQ24160_ADDRESS, REG_STATUS, rx_buff);
	
	unsigned char test_result = 0;
	if (rx_buff[0] == test_val) {
		test_result = 1;
		NRF_LOG_INFO("\r\nTest register: %02x OK\r\n", reg_addr);

	} else {
		NRF_LOG_INFO("\r\nTest register: %02x failed\r\n", reg_addr);
	}
	NRF_LOG_FLUSH();
...
int main() {
	...
	while (1) {
		...
		unsigned char test_value = 0xFF;
		unsigned char test_succeed = 1;
		test_succeed &= TestRegister(test_value--, REG_STATUS);
		test_succeed &= TestRegister(test_value--, REG_BR_STATUS);
		test_succeed &= TestRegister(test_value--, REG_CONTROL);
		test_succeed &= TestRegister(test_value--, REG_BR_VOLTAGE);
		test_succeed &= TestRegister(test_value--, REG_VENDOR);
		test_succeed &= TestRegister(test_value--, REG_TERMINATION);
		test_succeed &= TestRegister(test_value--, REG_DPM);
		test_succeed &= TestRegister(test_value--, REG_NTC);
